...
typedef struct
{
	...
	VkBuffer				faceBuffer;			/**memory handle for the face buffer (always 2 faces)*/
	VkDeviceMemory			faceBufferMemory;	/**memory handle for the face memory*/
...
VkDescriptorSetLayout * gf3d_sprite_get_descriptor_set_layout();
...
void gf3d_sprite_manager_init(Uint32 max_sprites, Uint32 chain_length, VkDevice device)
{
	...
	if (max_sprites == 0)
	{
		slog("cannot initilize sprite manager for 0 sprites");
		return;
	}
...
//	gf3d_sprite.pipe = gf3d_vgraphics_get_graphics_overlay_pipeline();
	...
	gf3d_vgraphics_create_buffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
	...
	gf3d_vgraphics_create_buffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 
	&gf3d_sprite.faceBuffer, &gf3d_sprite.faceBufferMemory);
...
Sprite *gf3d_sprite_get_by_filename(char *filename)
{
	...
	for (i = 0; i < gf3d_sprite.max_sprites; i++)
	{
		...
		if (gfc_line_cmp(gf3d_sprite.sprite_list[i].filename, filename))
		{
...
Sprite * gf3d_sprite_load(char * filename, int frame_width, int frame_height, Uint32 frames_per_line)
{
	...
	if (!sprite->texture)
	{
		slog("gf3d_sprite_load: failed to load a texture for sprite");
...
void gf3d_sprite_delete(Sprite *sprite)
{
	...
	{
		vkDestroyBuffer(gf3d_sprite.device, sprite->uniformBuffer[i], NULL);
		vkFreeMemory(gf3d_sprite.device, sprite->uniformBufferMemory[i], NULL);
	}
	if (sprite->uniformBuffer != VK_NULL_HANDLE)
	{
		vkDestroyBuffer(gf3d_sprite.device, sprite->uniformBuffer,NULL);
		slog("sprite %s vert buffer freed", sprite->filename);
	}
	if (sprite->uniformBufferMemory != VK_NULL_HANDLE)
	{
		vkFreeMemory(gf3d_sprite.device, sprite->uniformBuffer, NULL);
...
void gf3d_sprite_render(Sprite *sprite,VkCommandBuffer commandBuffer,VkDescriptorSet *descriptorSet)
{
	...
	vkCmdBindVertexBuffers(commandBuffer, 0, 1, &sprite->uniformBuffer, offsets);
...
void gf3d_sprite_draw(Sprite *sprite, Vector2D position, Vector2D scale,Uint32 frame, Uint32 buffer_frame, VkCommandBuffer commandBuffer)
{
	...
	gfc_matrix_scale(spriteMat,vector3d(scale.x,scale.y,1));
	gfc_matrix_new_translation(spriteMat,vector3d(position.x*2 /(float)extent.width, position.y*2/(float)extent.height, 0));

	gf3d_sprite_update_basic_descriptor_set(sprite, *descriptorSet, buffer_frame, spriteMat, frame);
...
void gf3d_sprite_update_basic_descriptor_set(Sprite *sprite, VkDescriptorSet descriptorSet, Uint32 chainIndex, Matrix4 spriteMat, Uint32 frame)
{
	...
	gf3d_sprite_update_uniform_buffer(sprite, chainIndex, spriteMat, frame);
	bufferInfo.buffer = sprite->uniformBuffer[chainIndex];
	bufferInfo.offset = 0;
...
void gf3d_sprite_create_uniform_buffer(Sprite *sprite)
{
	...
	sprite->uniformBuffer = (VkBuffer*)gfc_allocate_array(sizeof(VkBuffer), buffercount);
	sprite->uniformBufferMemory = (VkDeviceMemory*)gfc_allocate_array(sizeof(VkDeviceMemory), buffercount);
	sprite->uniformBufferCount = buffercount;

	for (i = 0; i < buffercount; i++)
	{
		gf3d_vgraphics_create_buffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, 
		&sprite->uniformBuffer[i], &sprite->uniformBufferMemory[i]);
	}
}
...
void gf3d_sprite_create_vertex_buffer(Sprite *sprite)
{
	...
	gf3d_vgraphics_create_buffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
		&sprite->uniformBuffer, &sprite->uniformBufferMemory);
	slog("Crashing here?");
	gf3d_vgraphics_copy_buffer(stagingBuffer, sprite->uniformBuffer, bufferSize);
...
void gf3d_sprite_update_uniform_buffer(Sprite *sprite, uint32_t currentImage, Matrix4 spriteMat, Uint32 frame)
{
	void* data;
	SpriteUBO ubo;
	gfc_matrix_copy(ubo.sprite, spriteMat);
	...
	vkMapMemory(gf3d_sprite.device, sprite->uniformBufferMemory[currentImage], 0, sizeof(SpriteUBO), 0, &data);

	memcpy(data, &ubo, sizeof(SpriteUBO));

	vkUnmapMemory(gf3d_sprite.device, sprite->uniformBufferMemory[currentImage]);
}
