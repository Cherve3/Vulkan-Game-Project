...
Model * gf3d_model_load_full(const char * modelFile,const char *textureFile)
{
    ...
    if (!model->texture)
    {
        model->texture = gf3d_texture_load("textures/default.png");
    }
...
void gf3d_model_draw_sky(Model *model,Matrix4 modelMat,Color color)
{
    ...
    gf3d_model_update_sky_model_descriptor_set(model,*descriptorSet,bufferFrame,modelMat, gfc_color_to_vector4(color));
